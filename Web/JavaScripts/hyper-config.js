(
   function () {
      let savedConfig = sessionStorage.getItem("__HYPER_CONFIG__");
      ...
      let layoutColor = this.html.getAttribute('data-theme');
      config['theme'] = layoutColor !== null ? layoutColor : defaultConfig.theme;

      let layoutNav = this.html.getAttribute('data-layout');
      if (layoutNav !== null) {
         config['nav'] = layoutNav === 'topnav' ? 'horizontal' : 'vertical';
      }
      else {
         config['nav'] = defaultConfig.nav;
      }

      let layoutSize = this.html.getAttribute('data-layout-mode');
      config['layout']['mode'] = layoutSize !== null ? layoutSize : defaultConfig.layout.mode;

      let layoutMode = this.html.getAttribute('data-layout-position');
      config['layout']['position'] = layoutMode !== null ? layoutMode : defaultConfig.layout.position;

      let topbarColor = this.html.getAttribute('data-topbar-color');
      config['topbar']['color'] = topbarColor != null ? topbarColor : defaultConfig.topbar.color;

      let leftbarSize = this.html.getAttribute('data-sidenav-size');
      config['sidenav']['size'] = leftbarSize !== null ? leftbarSize : defaultConfig.sidenav.size;

      let sidebarUser = this.html.getAttribute('data-sidenav-user')
      config['sidenav']['user'] = sidebarUser !== null ? true : defaultConfig.sidenav.user;

      let menuColor = this.html.getAttribute('data-menu-color');
